...
/**
 * A mixin for a text field that allows for autocompletion of text fields. This
 * is based on Prototype's autocompleter control.
 * <p/>
 * The mixin renders an (initially invisible) progress indicator after the field
 * (it will also be after the error icon most fields render). The progress
 * indicator is made visible during the request to the server. The mixin then
 * renders a &lt;div&gt; that will be filled in on the client side with
 * dynamically obtained selections.
 * <p/>
 * Multiple selection on the client is enabled by binding the tokens parameter
 * (however, the mixin doesn't help split multiple selections up on the server,
 * that is still your code's responsibility).
 * <p/>
 * The container is responsible for providing an event handler for event
 * "providecompletions". The context will be the partial input string sent from
 * the client. The return value should be an array or list of completions, in
 ...
 * @tapestrydoc
 */
@ImportJQueryUI({ "jquery.ui.widget", "jquery.ui.position",
...
public class CustomAutoComplete {
	...
	Object onAutocomplete() {
		...
		JSONObject extra = (json.length() > 1) ? new JSONObject(
				json.getString(EXTRA_NAME)) : new JSONObject();
		...
		Object[] params = extra.length() == 0 ? new Object[] { input }
				: new Object[] { input, extra };

		resources.triggerEvent(EventConstants.PROVIDE_COMPLETIONS, params,
				callback);

		ContentType contentType = responseRenderer.findContentType(this);

		return new TextStreamResponse(contentType.toString(),
				generateResponseJSON(matchesHolder.get()).toString());
	}
